...
class AiViewModel : ViewModel() {
    ...
    fun fetchAdvice(userName: String, currentBudgets: List<BudgetProgress>) {
        ...
        viewModelScope.launch {
            ...
            val monthProgress = (currentDay.toFloat() /
                    daysInMonth.toFloat() * 100).toInt()
            ...
            val currentTotal = accounts.filter { it.selected }.sumOf {
                it.balance
            }
            ...
            val budgetStatus = currentBudgets.joinToString {
                val name =
                    ...
                    it.budget.categoryName.lowercase().replaceFirstChar { c ->
                        c.uppercase()
                    }
                "$name: ${(it.progress * 100).toInt()}%"
            }

            val dailyBurnRate = if (currentDay > 0) expense /
                    currentDay else 0.0
            val projectedExpense = dailyBurnRate * daysInMonth
            val incomeShortfall = if (projectedExpense > income)
                projectedExpense - income else 0.0
            ...
            DATA: Income ${income.toInt()}, Spent ${expense.toInt()}, Wallet ${currentTotal.toInt()}.
            BUDGETS: $budgetStatus.
            DREAM: ${dream?.title}.
            RECENT: $recentTxsSummary.
            
            TASK: Provide a professional CFO audit. Max 3 short sentences. NO numbers.
